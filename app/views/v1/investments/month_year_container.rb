...
class Views::V1::Investments::MonthYearContainer < Views::Base
  ...
  def view_template
    turbo_frame_tag :month_year_container do
      ...
      active_month_years.sort.each do |month_year|
        turbo_frame_tag "month_year_container_#{month_year}", src: month_year_v1_investments_path(custom_params.merge(month_year:))
      end
    end
  end
end
