...
class Views::V1::CashInstallments::Index < Views::Base # rubocop:disable Metrics/ClassLength
  ...
  def render_mobile_cash_installment(cash_installment, cash_transaction, style, avatar_name) # rubocop:disable Metrics/PerceivedComplexity,Metrics/CyclomaticComplexity
    turbo_frame_tag dom_id cash_installment do
      ...
      render Views::V1::CashInstallments::PayModal.new(cash_installment:) if should_display_link_to_pay || cash_transaction.card_payment?
            ...
            div(class: "flex-1 flex items-center justify-between gap-1 min-w-0") do
              if cash_transaction.investment?
                ...
                link_to cash_transaction.description,
                        v1_investments_path(investment:, default_year:, active_month_years:, format: :turbo_stream),
                ...
                link_to cash_transaction.description,
                        v1_card_transactions_path(user_card_id: cash_transaction.user_card_id, default_year:, active_month_years:, format: :turbo_stream),
                        class: "cash_transaction_description truncate text-md underline underline-offset-[3px]",
                        data: { turbo_frame: :center_container, turbo_prefetch: false }
              else
                link_to cash_transaction.description, edit_v1_cash_transaction_path(cash_transaction),
          ...
          div(class: "flex flex-wrap items-center gap-1") do
            ...
            div(class: "flex flex-wrap justify-end gap-2 ml-auto", data: { datatable_target: :entity, id: cash_transaction.entities.map(&:id) }) do
              cash_transaction.entity_transactions.order(:id).map(&:entity).each do |entity|
                Link(
                  href: new_v1_cash_transaction_path(cash_transaction: { entity_id: entity.id }, format: :turbo_stream),
                  size: :xs,
      ...
      div(
        ...
        div(class: "col-span-4 flex-1 flex items-center justify-between gap-1 min-w-0 mx-2") do
          if cash_transaction.investment?
            ...
            link_to cash_transaction.description,
                    edit_v1_cash_transaction_path(cash_transaction),
                    id: "edit_cash_transaction_#{cash_transaction.id}",
                    class: "cash_transaction_description flex-1 truncate text-md underline underline-offset-[3px]",
                    data: { turbo_frame: :center_container }
          end
