...
class Views::HealthCheck::Dashboard::CheckCard < Views::Base
  ...
  def view_template
    article(
      ...
      class: "flex min-w-0 flex-col rounded-lg border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900 dark:shadow-none"
    ) do
      card_header
      card_reason
      counts
  ...
  def counts
    dl(class: "mt-4 grid grid-cols-2 gap-2 sm:grid-cols-5") do
  ...
  def metadata
    div(class: "mt-4 grid gap-3 border-t border-slate-200 pt-3 text-xs sm:grid-cols-3 dark:border-slate-700") do
