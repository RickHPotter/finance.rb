...
class Views::CashTransactions::Show < Views::Base
    ...
    div(class: "flex flex-col gap-4 border-b border-slate-200 pb-5 lg:flex-row lg:items-start lg:justify-between") do
      div do
        ...
        p(class: "mt-3 max-w-3xl text-sm text-slate-600") do
          I18n.t("dashboards.cash_transactions.placeholder")
        end
      end

      div(class: "flex flex-wrap gap-2") do
        link_to(action_model(:edit, CashTransaction), edit_cash_transaction_path(cash_transaction),
        ...
        link_to(action_model(:index, CashTransaction, 2), cash_transactions_path,
  ...
  def placeholder_grid
    div(class: "mt-6 grid gap-3 md:grid-cols-3") do
      dashboard_stat(model_attribute(CashTransaction, :price), from_cent_based_to_float(cash_transaction.price, "R$"))
      dashboard_stat(model_attribute(CashTransaction, :cash_installments_count), cash_transaction.cash_installments_count)
      dashboard_stat(model_attribute(CashTransaction, :user_bank_account_id), cash_transaction.user_bank_account&.user_bank_account_name || "-")
    end
  end
