...
class Views::Admin::Settings::CardExchangeProjectionAudit < Views::Base
    ...
    turbo_frame_tag :settings_card_exchange_projection_audit_content do
      div(class: "space-y-4 text-left text-black") do
        div(class: "rounded-2xl border border-slate-200 bg-white p-4 shadow-sm") do
          h2(class: "text-lg font-bold text-slate-900") { I18n.t("settings.card_exchange_projection_audit.title") }
          p(class: "mt-1 text-sm text-slate-600") { I18n.t("settings.card_exchange_projection_audit.description") }
  ...
  def audit_card(row)
    div(class: "overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm") do
      ...
      div(class: "grid gap-4 border-t border-slate-200 px-4 py-4 lg:grid-cols-2") do
  ...
  def rows_column(title, rows_collection, expected:)
    div(class: "space-y-2") do
      ...
      rows_collection.each do |entry|
        div(class: "rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-800") do
          ...
          if expected
            p(class: "text-xs text-slate-600") { "#{I18n.t('settings.card_exchange_projection_audit.installment_number')}: #{entry[:number]}" }
          else
            p(class: "text-xs text-slate-600") { "Exchange ##{entry[:id]} · Cash ##{entry[:cash_transaction_id]} · #{entry[:entity_name]}" }
          end
        end
      end
    end
  ...
  def metric(label, value)
    div(class: "rounded-xl border border-slate-200 bg-white px-3 py-3") do
      ...
      p(class: "mt-2 text-lg font-bold text-slate-900") { from_cent_based_to_float(value, "R$") }
    end
  end
  ...
  def allocation_issue_row(allocation_row)
    div(class: "mt-3 rounded-xl border border-rose-200 bg-white px-3 py-2 text-sm text-slate-800") do
      div(class: "flex flex-wrap items-start justify-between gap-2") do
        div(class: "space-y-1") do
          ...
          p(class: "text-xs text-slate-600") { I18n.t("settings.card_exchange_projection_audit.issue_codes.#{allocation_row[:issue_code]}") }
        end
