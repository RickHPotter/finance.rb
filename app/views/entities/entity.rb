      ...
      div(class: "p-4") do
        div(class: "grid grid-cols-2 gap-4 my-2") do
          div(class: "space-y-1") do
            ...
            div(class: "flex items-center") { span(class: "text-sm font-semibold text-slate-800 dark:text-slate-100") { entity.card_transactions_count } }
          end
          ...
          div(class: "space-y-1") do
            ...
            div(class: "flex items-center") do
              span(class: "text-sm font-semibold text-slate-800 mr-auto dark:text-slate-100") { from_cent_based_to_float(entity.card_transactions_total, "R$") }
            end
          end
        end
        ...
        div(class: "grid grid-cols-2 gap-4 my-2") do
          div(class: "space-y-1") do
            div(class: "flex items-center text-sm font-medium text-slate-500") do
              cached_icon :number
              span(class: "ml-2 truncate dark:text-slate-400") { pluralise_model(CashTransaction, 2) }
            end

            div(class: "flex items-center") { span(class: "text-sm font-semibold text-slate-800 dark:text-slate-100") { entity.cash_transactions_count } }
          end
          ...
          div(class: "space-y-1") do
            ...
            div(class: "flex items-center") do
              span(class: "text-sm font-semibold text-slate-800 mr-auto dark:text-slate-100") { from_cent_based_to_float(entity.cash_transactions_total, "R$") }
            end
          end
        end
        ...
        div(class: "mt-4 flex justify-end gap-2 border-t border-slate-200 pt-3 dark:border-slate-700") do
          Button(
            link: search_card_transactions_path(card_transaction: { entity_id: entity.id }, all_month_years: true),
            variant: :outline,
          ...
          Button(
            link: cash_transactions_path(cash_transaction: { entity_id: entity.id }, all_month_years: true),
            variant: :outline,
            class: "border-slate-300 text-slate-700 hover:bg-slate-100 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800",
            data: { turbo_frame: "_top", turbo_prefetch: false }
          ) do
