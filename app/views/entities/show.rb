...
class Views::Entities::Show < Views::Base # rubocop:disable Metrics/ClassLength
    ...
    div(class: "flex flex-col gap-5 border-b border-slate-200 pb-5 dark:border-slate-700 lg:flex-row lg:items-start lg:justify-between") do
      div(class: "min-w-0 text-left") do
        div(class: "flex items-center gap-3") do
          image_tag(asset_path("avatars/#{entity.avatar_name}"), class: "size-12 rounded-full border border-slate-200 bg-white dark:border-slate-700")
          h1(class: "text-3xl font-black tracking-tight text-slate-950 dark:text-slate-100 sm:text-4xl") { entity.name }
        end
  ...
  def details_section
    section_card("Details") do
      div(class: "grid gap-3 sm:grid-cols-2 xl:grid-cols-4") do
        dashboard_stat(model_attribute(Entity, :entity_name), entity.name)
        dashboard_stat(model_attribute(Entity, :status), entity.active? ? model_attribute(Entity, "statuses.active") : model_attribute(Entity, "statuses.inactive"))
  ...
  def destroy_action
    return unless entity_destroyable?
  ...
  def status_badge
    colour = entity.active? ? "bg-emerald-100 text-emerald-900" : "bg-slate-200 text-slate-700"
    label = entity.active? ? model_attribute(Entity, "statuses.active") : model_attribute(Entity, "statuses.inactive")
  ...
  def scoped_cash_transactions
    @scoped_cash_transactions ||= current_context.cash_transactions.joins(:entities).where(entities: { id: entity.id }).distinct
  end

  def scoped_card_transactions
    @scoped_card_transactions ||= current_context.card_transactions.joins(:entities).where(entities: { id: entity.id }).distinct
  end
