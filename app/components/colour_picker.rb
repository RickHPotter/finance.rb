    ...
    def view_template
      div(class: "relative", data: { controller: "colour-picker" }) do
        div(class: "relative", data: { action: "click->colour-picker#toggle" }) do
          ...
          div(class: "w-10 h-10 rounded-full border-1 #{@colour}", data: { colour_picker_target: "colourIndicator" })
        end
        ...
        div(class: "hidden absolute z-50 w-48 mt-2 p-2 left-1/2 -translate-x-1/2 bg-white rounded-lg shadow-lg border border-zinc-300 grid grid-cols-4 gap-2",
          ...
          ::COLOURS.each_pair do |name, classes|
            button type: "button",
                   class: "w-10 h-10 rounded shadow-lg border hover:scale-125 transition-all ease-in-out duration-100 cursor-pointer
