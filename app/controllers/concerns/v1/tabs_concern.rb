...
module V1
  module TabsConcern
    ...
    def set_basic_sublinks
      ...
      @basic_tab = [
        Item.new(t("tabs.user_bank_account"), :bank,        v1_user_bank_accounts_path, @active_sub_menu == :user_bank_account),
        Item.new(t("tabs.user_card"),         :credit_card, v1_user_cards_path,         @active_sub_menu == :user_card),
        Item.new(t("tabs.category"),          :category,    v1_categories_path,         @active_sub_menu == :category),
        Item.new(t("tabs.entity"),            :user_circle, v1_entities_path,           @active_sub_menu == :entity),
        Item.new(t("tabs.conversation"),
                 :message,
                 v1_conversation_path(1),
    ...
    def set_card_transaction_sublinks
      ...
      @card_transaction_tab = user_cards.map do |user_card_id, user_card_name|
        ...
        Item.new(user_card_name, :credit_card, v1_card_transactions_path(user_card_id:), default)
      end

      if @card_transaction_tab.present?
        @card_transaction_tab << Item.new(
          action_message(:search), :magnifying_glass, search_v1_card_transactions_path, @active_sub_menu.to_sym == :search
        )
        return
      end

      @card_transaction_tab << Item.new(action_model(:new, UserCard), "credit_card", new_v1_user_card_path, false)
    end
    ...
    def set_cash_transaction_sublinks
      ...
      @cash_transaction_tab = [
        Item.new(t("tabs.pix"),        :mobile,      v1_cash_transactions_path, @active_sub_menu == :pix, cash_notification_type),
        Item.new(t("tabs.budget"),     :piggy_bank,  v1_budgets_path,           @active_sub_menu == :budget),
        Item.new(t("tabs.investment"), :trending_up, v1_investments_path,       @active_sub_menu == :investment),
        Item.new(t("tabs.balance"),    :chart,       v1_balances_path,          @active_sub_menu == :balance)
      ]
    end
  end
end
