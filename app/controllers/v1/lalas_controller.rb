...
class V1::LalasController < V1::ApplicationController
  ...
  def set_variables
    @main_items = [ { label: t("tabs.pix"), icon: :mobile, link: v1_lalas_cash_transactions_path, default: @active_menu == :pix } ]
    ...
    card_items = User.first.user_cards.active.pluck(:id, :user_card_name).map do |user_card_id, user_card_name|
      ...
      { label: user_card_name, icon: :credit_card, link: v1_lalas_card_transactions_path(user_card_id:), default: }
    end
