...
module Logic
  class References
    ...
    def self.merge(user_card, source_reference_date, target_reference_date, merge_mode:, context: user_card.user.main_context)
      merge_result(user_card, source_reference_date, target_reference_date, merge_mode:, context:).applied?
    end
    ...
    def self.merge_result(user_card, source_reference_date, target_reference_date, merge_mode:, context: user_card.user.main_context)
      ...
      return reallocation_result(user_card, source_date, target_date, context:) if merge_mode == REALLOCATE_INSTALLMENTS

      ReferenceMerges::CombineApply.new(user_card:, context:, source_date:, target_date:).call
    end
