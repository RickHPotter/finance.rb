...
module Logic
  class RecalculateCountAndTotalService
    ...
    def recalculate_card_transaction
      ...
      categories = @card_transaction.categories + @card_transaction.category_transactions.map(&:category)
      entities   = @card_transaction.entities + @card_transaction.entity_transactions.map(&:entity)

      user_card&.update_columns(card_transactions_total: user_card.card_transactions.sum(:price))
      categories.uniq.each(&:update_card_transactions_count_and_total)
      entities.uniq.each(&:update_card_transactions_count_and_total)
    end

    def recalculate_cash_transaction
      user_bank_account = @cash_transaction.user_bank_account
      categories        = @cash_transaction.categories + @cash_transaction.category_transactions.map(&:category)
      entities          = @cash_transaction.entities + @cash_transaction.entity_transactions.map(&:entity)

      user_bank_account&.update_columns(cash_transactions_total: user_bank_account.cash_transactions.sum(:price))
      categories.uniq.each(&:update_cash_transactions_count_and_total)
      entities.uniq.each(&:update_cash_transactions_count_and_total)
    end
  end
end
