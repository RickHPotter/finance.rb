...
RSpec.describe "CardTransactionFlow", class: CardTransaction, type: :model do
  let!(:ct_with_categories) { FactoryBot.create(:card_transaction, :random) }
  ...
  describe "[ business logic ]" do
    ...
    #
    context "( card_transaction creation with category_transaction )" do
      it "creates the corresponding category_transaction" do
        expect(ct_with_categories.custom_categories.count).to eq(1)
      end
    end

    context "( card_transaction with category_transactions updates )" do
      before { ct_with_categories.update(category_transactions: []) }

      it "destroys the existing category_transactions when emptying category_transactions" do
      ...
      it "destroys the existing category_transactions and then creates them again" do
        ct_with_categories.update(category_transactions: FactoryBot.build_list(:category_transaction, 2, :random))
