...
RSpec.describe "Entity merges" do
  ...
  describe "POST /entities/:id/merge" do
    ...
    it "applies the merge and redirects via Turbo Stream action" do
      ...
      # Should redirect
      expect(response.body).to include('turbo-stream action="redirect" target="/custom"')

      # Should include flash
      expect(response.body).to include('turbo-stream action="update" target="notification"')
      expect(response.body).to include(I18n.t("entity_merges.applied", default: "Entity merged successfully"))
