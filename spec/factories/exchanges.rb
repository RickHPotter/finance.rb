...
FactoryBot.define do
  factory :exchange do
    ...
    transaction_entity { custom_create(:transaction_entity, options: { is_payer: true }) }

    trait :different do
      exchange_type { 1 }
      amount_to_be_returned { '9.99' }
      amount_returned { '9.99' }
      transaction_entity { different_custom_create(:transaction_entity, options: { is_payer: true }) }
    end
    ...
    trait :random do
      ...
      transaction_entity { random_custom_create(:transaction_entity, options: { is_payer: true }) }
    end
  end
end
