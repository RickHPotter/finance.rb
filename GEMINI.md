...
For mobile finance forms, prefer the shared `Views::Shared::DatetimeInput` with RubyUI calendar/clock modes instead of adding one-off native date/time inputs. When a form has a matching submission skeleton, keep the skeleton layout aligned with the real mobile control, especially calendar/clock swaps.
For Turbo form failures that need detailed feedback, prefer `failure_notifications_for(record, notification, Model)`. Stream the first alert with `turbo_stream.update(:notification, ...)` and the remaining alerts with `turbo_stream.append(:notification, ...)` so the generic failure and concrete validation/history messages stack separately.
Keep notification positioning centralized in the `turbo-frame#notification` container. The shared flash partial should render as a normal stack child, not as its own fixed/inset overlay, otherwise multiple failure notifications render on top of each other.
For finance transaction forms, preserve the distinction between cash and card behavior: cash date changes update unpaid installment dates locally, while card date/user-card changes may intentionally submit a hidden update refresh so billing-reference calculations stay correct.
...
In Phlex views, use string literals for DOM `id` values whenever JavaScript, Stimulus, labels, or tests depend on an exact ID. Symbol IDs may be normalized to kebab-case in the rendered HTML, which can break underscore-based selectors.
