...
## Slice 13 — Profile Schema Completeness: `page_density` and Avatar
...
#### 13a. `page_density` Preference

- **Migration:** `add_column :user_preferences, :page_density, :string, null: false, default: "comfortable"`.
- **Model:** Add `enum :page_density, { comfortable: "comfortable", compact: "compact" }, suffix: true` and include it in the presence validation.
- **Layout:** Apply the value as a `data-page-density` attribute on the `<body>` tag so Tailwind CSS classes or custom styles can respond to it.
- **UI:** Add the toggle to the existing Preferences form in `Views::Profiles::Edit`.

#### 13b. Avatar / Profile Picture
